...
export function ApplicationDetailsDialog({
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      setError(null);
      try {
        const response = await api.get<{ application: ApplicationRecord }>(
          `/api/applications/${initialApplication.id}`,
        );
        ...
        if (response.success && response.data?.application) {
          ...
          setError(
            response.message || "დეტალური ინფორმაციის ჩატვირთვა ვერ მოხერხდა",
          );
        }
      } catch (err) {
        setError(
          getApiErrorMessage(err) ||
            "დეტალური ინფორმაციის ჩატვირთვა ვერ მოხერხდა",
        );
      } finally {
        setLoading(false);
      }
          ...
          <div className="flex items-center justify-between pr-6">
            <div>
              <DialogTitle className="text-xl">
                განაცხადი #
                {initialApplication.application_no || initialApplication.id}
              </DialogTitle>
              <DialogDescription>
            ...
            <Badge
              variant={
                initialApplication.status === "Draft" ? "outline" : "default"
              }
              className="h-6 mr-10"
            >
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <section className="space-y-4">
                  ...
                  <div className="grid gap-4 p-4 rounded-xl border bg-slate-50/50">
                    ...
                    <div className="grid gap-1">
                      ...
                      <p className="text-sm font-medium">
                        {application.officer.first_name}{" "}
                        {application.officer.last_name}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        ID: {application.officer.id} (@
                        {application.officer.username})
                      </p>
                    </div>
                  ...
                  <div className="grid gap-4 p-4 rounded-xl border bg-slate-50/50">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="grid gap-1">
                        ...
                        <p className="text-sm font-medium">
                          {application.loan_term_months} თვე
                        </p>
                      </div>
                      <div className="grid gap-1">
                        <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t pt-3">
                      <div className="grid gap-1">
                        ...
                        <p className="text-sm font-medium">
                          {formatAmount(application.commission_fee)}{" "}
                          {application.commission_fee_type === "percent"
                            ? "%"
                            : application.currency}
                        </p>
                      </div>
                      <div className="grid gap-1">
                        ...
                        <p className="text-sm font-medium">
                          {formatAmount(application.down_payment)}{" "}
                          {application.currency}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="grid gap-4 p-4 rounded-xl border bg-slate-50/50">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="grid gap-1">
                        ...
                        <p className="text-sm font-medium">
                          {formatAmount(application.seller_price)}{" "}
                          {application.currency}
                        </p>
                      </div>
                      <div className="grid gap-1">
                        ...
                        <p className="text-sm font-bold text-emerald-700">
                          {formatAmount(application.market_value)}{" "}
                          {application.currency}
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 border-t pt-3">
                      <div className="grid gap-1">
                        ...
                        <p
                          className={`text-sm font-bold ${application.ltv_warning ? "text-red-600" : "text-slate-900"}`}
                        >
                          {application.ltv}%
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t pt-3">
                      ...
                      <div className="grid gap-1">
                        ...
                        <p className="text-sm font-medium">
                          {formatAmount(application.liquidation_value)}{" "}
                          {application.currency}
                        </p>
                      </div>
                    </div>
                ...
                {application.has_coborrower_guarantor === "1" ||
                application.has_coborrower_guarantor === 1 ||
                application.co_borrower_name ||
                    ...
                    <div className="grid gap-4 p-4 rounded-xl border bg-slate-50/50 sm:grid-cols-2">
                      <div className="grid gap-1">
                        ...
                        {application.co_borrower_personal_id && (
                          <p className="text-xs text-muted-foreground">
                            P/N: {application.co_borrower_personal_id}
                          </p>
                        )}
                      </div>
                      <div className="grid gap-1">
                        ...
                        {application.guarantor_personal_id && (
                          <p className="text-xs text-muted-foreground">
                            P/N: {application.guarantor_personal_id}
                          </p>
                        )}
                      </div>
                    </div>
